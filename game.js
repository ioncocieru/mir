...
let quadPanelOpen = false;
let quadPanelRank = null;
let quadPanelCapId = null; // null = fără carte în plus
...
function takePileLogic(state, playerId, requestedCount) {
  ...
  const leftBehind = state.pile.slice(0, state.pile.length - count); // rămân pe masă

  player.hand.push(...taken);
  state.pile = leftBehind;

  const extra = leftBehind.length > 0 ? ` — rămân ${leftBehind.length} cărți pe masă.` : " — masa rămâne goală.";
...
function playQuadLogic(state, playerId, quadRank, capCardId) {
  const player = state.players.find(p => p.id === playerId);
  if (!player || player.out || player.claimedFinished) return null;
  if (state.order[state.turnIndex] !== playerId) { showToast("Nu e rândul tău."); return null; }

  const quadCards = player.hand.filter(c => c.rank === quadRank);
  if (quadCards.length !== 4) { showToast("Nu ai toate cele 4 cărți de acest fel."); return null; }

  if (state.mustOpenWithSixHearts) {
    if (quadRank !== 6) { showToast("Trebuie să deschizi cu 6 de inimă."); return null; }
  } else if (state.pile.length > 0) {
    const topCard = state.pile[state.pile.length - 1];
    if (quadRank < topCard.rank) { showToast("Careul trebuie să fie egal sau mai mare decât ultima carte."); return null; }
  }

  let capCard = null;
  if (capCardId) {
    capCard = player.hand.find(c => c.id === capCardId && c.rank !== quadRank);
    if (!capCard) { showToast("Cartea aleasă pe deasupra nu e validă."); return null; }
    if (capCard.rank <= quadRank) { showToast("Cartea de deasupra trebuie să fie mai mare decât careul."); return null; }
  }

  player.hand = player.hand.filter(c => c.rank !== quadRank && (!capCard || c.id !== capCard.id));
  state.pile.push(...quadCards);
  if (capCard) state.pile.push(capCard);
  state.mustOpenWithSixHearts = false;

  const capText = capCard ? ` și pune deasupra ${cardLabel(capCard)}${suitMeta(capCard.suit).symbol}!` : ".";
  state.chat.push({ author: "Sistem", text: `🃏 ${player.name} pune careul de ${RANK_LABEL[quadRank]}${capText}`, ts: nowIso(), system: true });
...
$("#btn-quad-open").addEventListener("click", () => {
  const me = latestState && latestState.players.find(p => p.id === myId);
  if (!me) return;
  const ranks = findFourOfAKindRanks(me.hand);
  if (!ranks.length) return;
  quadPanelRank = ranks[0];
  quadPanelCapId = null;
  quadPanelOpen = true;
  render(latestState);
});
$("#btn-quad-cancel").addEventListener("click", () => {
  quadPanelOpen = false;
  render(latestState);
});
$("#btn-quad-confirm").addEventListener("click", async () => {
  const rank = quadPanelRank;
  const capId = quadPanelCapId;
  quadPanelOpen = false;
  await updateRoomState((state) => playQuadLogic(state, myId, rank, capId));
});
...
function renderGame(state) {
  ...
  // careu (4 cărți identice) — se pot pune toate deodată, doar la rândul tău
  const topCard = state.pile.length ? state.pile[state.pile.length - 1] : null;
  const quadRanks = (me && !me.out && isMyTurn) ? findFourOfAKindRanks(me.hand).filter(r => {
    if (state.mustOpenWithSixHearts) return r === 6;
    return !topCard || r >= topCard.rank;
  }) : [];
  if (!quadRanks.length) quadPanelOpen = false;
  $("#btn-quad-open").classList.toggle("hidden", quadRanks.length === 0 || quadPanelOpen);
  if (quadRanks.length) $("#btn-quad-open").textContent = `🃏 Pune careul de ${RANK_LABEL[quadRanks[0]]}`;

  const quadPanel = $("#quad-panel");
  quadPanel.classList.toggle("hidden", !quadPanelOpen);
  if (quadPanelOpen) {
    if (!quadRanks.includes(quadPanelRank)) quadPanelRank = quadRanks[0];
    $("#quad-panel-title").textContent = `Careu de ${RANK_LABEL[quadPanelRank]} — adaugi o carte mai mare pe deasupra?`;
    const optionsEl = $("#quad-cap-options");
    optionsEl.innerHTML = "";
    const noneChip = el("div", "quad-cap-chip none-option" + (quadPanelCapId === null ? " selected" : ""), "Fără carte în plus");
    noneChip.addEventListener("click", () => { quadPanelCapId = null; render(latestState); });
    optionsEl.appendChild(noneChip);
    me.hand.filter(c => c.rank > quadPanelRank).sort((a, b) => a.rank - b.rank).forEach(c => {
      const chip = renderCardEl(c, false);
      chip.classList.add("quad-cap-chip");
      if (quadPanelCapId === c.id) chip.classList.add("selected");
      chip.addEventListener("click", () => { quadPanelCapId = (quadPanelCapId === c.id ? null : c.id); render(latestState); });
